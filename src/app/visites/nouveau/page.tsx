...
import { ArrowLeft, Camera, Binoculars, Info, UserPlus, Search, MapPin, User } from 'lucide-react'
...
function NouvelleVisiteForm() {
  ...
  async function start() {
    ...
    // Nouveau prospect : on le crée d'abord (statut "nouveau"), puis on y rattache la visite.
  ...
  return (
    <div className="space-y-5 max-w-lg animate-fade-up">
      <Link href="/visites" className="inline-flex items-center gap-1.5 text-sm font-medium text-gray-500 hover:text-gray-700">
        <ArrowLeft className="w-4 h-4" /> Retour aux visites
      </Link>

      {/* En-tête illustré */}
      <div className="flex items-center gap-3">
        <span className="grid place-items-center w-12 h-12 rounded-2xl bg-gradient-to-br from-[#FF8A2B] to-[#FF6A00] text-white shadow-[var(--shadow-brand)] flex-shrink-0">
          <Binoculars className="w-6 h-6" />
        </span>
        <div>
          <h1 className="text-2xl font-bold font-heading text-marine leading-tight">Nouvelle visite de repérage</h1>
          <p className="text-gray-500 text-sm">Chez un prospect : photos + notes, avant le devis.</p>
        </div>
      </div>
      ...
      <Card className="border-0 shadow-[var(--shadow-md)]">
        <CardContent className="p-5 space-y-5">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="address" className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5 text-gray-400" /> Adresse du chantier</Label>
          ...
          {/* Nom de la visite */}
          <div className="space-y-1.5">
            <Label htmlFor="title">Nom de la visite <span className="text-gray-400 font-normal">(optionnel)</span></Label>
