...
import { Star, Search, ExternalLink, Check, Loader2, Pencil } from 'lucide-react'
...
export default function ReviewLinkGuide({ initialUrl, collapsible = false }: { initialUrl: string; collapsible?: boolean }) {
  ...
  async function save() {
    ...
    const { data, error } = await supabase.from('companies')
      .update({ google_review_url: url.trim() || null }).eq('user_id', user.id).select('id')
    setSaving(false)
  ...
  // Mode replié : le lien est déjà configuré, on montre juste un rappel + « Modifier ».
  if (collapsible && !open) {
    return (
    ...
    <Card className="border-0 shadow-[var(--shadow-sm)] ring-1 ring-amber-100">
      ...
      <CardContent className="px-4 pb-4 space-y-4">
        <p className="text-sm text-gray-500">
          Récupérez le lien « laisser un avis » de votre fiche Google — une seule fois. Ensuite, vos clients de chantiers terminés apparaissent ci-dessous, prêts à être sollicités en un clic.
        </p>
        ...
        <div className="flex gap-3">
          ...
          <div className="flex-1 space-y-2">
            ...
            <p className="text-xs text-gray-400">Connecté à votre compte Google, les fiches d&apos;établissement que vous gérez s&apos;affichent directement.</p>
          </div>
        </div>
