...
export default function ClientsKanban({ initialItems }: { initialItems: ClientCard[] }) {
  ...
  return (
    <DndKanban
      ...
      items={items}
      onMove={move}
      footer={<p className="text-[11px] text-gray-400 mt-3">Glissez une carte d&apos;une colonne à l&apos;autre, ou utilisez le menu de statut.</p>}
      renderCard={(c) => {
          ...
          <Card className="border-0 shadow-[var(--shadow-sm)] overflow-hidden cursor-grab active:cursor-grabbing" style={{ backgroundColor: `${dot}0A` }}>
            ...
            <CardContent className="p-3">
              ...
              <div className="mt-2.5 flex items-center justify-between gap-2 text-xs">
                <span className="text-gray-500">Facturé <b className="text-marine tabular-nums">{c.facture}</b></span>
                {c.reste && <span className="text-[#8A5A08] font-medium tabular-nums flex-shrink-0">Reste {c.reste}</span>}
              </div>

              <div className="mt-2.5 flex items-center gap-2 text-[11px] text-gray-400">
                <span className="flex items-center gap-1"><HardHat className="w-3 h-3" />{c.chantiers}</span>
                <span className="text-gray-300">·</span>
                <span className="truncate">{c.contact}</span>
              </div>

              <div className="mt-2.5">
                <ClientPhaseSelect clientId={c.id} current={c.status} />
              </div>
            </CardContent>
          </Card>
        )
