 ...
 *  - Un bouton « Mode vocal » distinct passe en conversation vocale en direct
 *    (réponses parlées, actions à la voix), dans la même case.
...
import {
  ...
  X, FileText, ReceiptText, Square, Volume2, ExternalLink, AudioLines, Keyboard,
} from 'lucide-react'
...
export default function DashboardAssistant({ onClose, demoSeed, initialMode }: { onClose?: () => void; demoSeed?: ChatMsg[]; initialMode?: 'chat' | 'voice' }) {
  ...
  const vsRef = useRef<VoiceState>('listening')
  ...
  const speak = useCallback((text: string) => {
    ...
    try {
      ...
      if (v) u.voice = v
      u.onend = () => { speakEndRef.current = Date.now(); if (modeRef.current === 'voice' && vsRef.current === 'speaking') setVS('listening') }
      setVS('speaking')
      synth.speak(u)
    } catch { setVS('listening') }
  }, [])
  ...
  const ask = useCallback(async (question: string) => {
    ...
    try {
      ...
      setLoading(false)
      if (modeRef.current === 'voice' && vsRef.current === 'thinking') setVS('listening')
    }
  }, [loading, speak])
  ...
  const startRec = useCallback(() => { const rec = recRef.current; if (!rec || runningRef.current) return; try { rec.start(); runningRef.current = true } catch {} }, [])
  const enterVoice = useCallback(() => {
    stopDictation()
    modeRef.current = 'voice'; setMode('voice'); setHeard(''); setVS('listening')
    const SR = getSR()
    if (!SR) return
    const rec = new SR()
    rec.lang = 'fr-FR'; rec.interimResults = true; rec.continuous = true
    rec.onresult = (e: any) => {
      ...
      if (itm && vsRef.current === 'listening') setHeard(itm)
      if (!final.trim()) return
      const spoken = lastSpokenRef.current.toLowerCase(); const f = final.toLowerCase().trim()
      const nearSpeech = vsRef.current === 'speaking' || (Date.now() - speakEndRef.current) < 900
      if (nearSpeech && f.length > 4 && spoken.includes(f.slice(0, Math.min(20, f.length)))) return
      if (vsRef.current === 'speaking') { try { window.speechSynthesis?.cancel() } catch {} }
      setHeard(final)
      if (pendingIdxRef.current != null && isAffirm(final)) { confirmPending(pendingIdxRef.current); return }
      ask(final)
    }
    rec.onerror = (ev: any) => { if (ev?.error === 'not-allowed' || ev?.error === 'service-not-allowed') exitVoice() }
    rec.onend = () => { runningRef.current = false; if (modeRef.current === 'voice') setTimeout(startRec, 250) }
    recRef.current = rec; startRec()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [ask, confirmPending, startRec, stopDictation])
  const exitVoice = useCallback(() => {
    modeRef.current = 'chat'; setMode('chat'); setHeard('')
    try { recRef.current?.abort() } catch {}; recRef.current = null; runningRef.current = false
  ...
  const vStatus = voiceState === 'listening' ? 'Je t’écoute…' : voiceState === 'thinking' ? 'Je réfléchis…' : 'Je te réponds…'
  ...
  return (
    ...
    >
      {/* En-tête */}
      <div className="flex items-center gap-2.5 px-3.5 py-3 text-white" style={{ background: 'radial-gradient(120% 160% at 100% 0%, #241a10 0%, #17130d 45%, #0c0c0e 100%)' }}>
        <span className={`grid h-9 w-9 place-items-center rounded-xl bg-[#F5A623] text-black ${mode === 'voice' ? 'animate-pulse' : ''}`}><Sparkles className="h-5 w-5" /></span>
        ...
        {mode === 'chat' ? (
          <button onClick={enterVoice} className="inline-flex items-center gap-1.5 h-8 rounded-full bg-[#F5A623] pl-2.5 pr-3 text-[13px] font-semibold text-black hover:brightness-105" title="Conversation vocale en direct">
      ...
      {mode === 'voice' ? (
        /* ─── Vue mode vocal (conversation en direct, dans la même case) ─── */
        <div className="relative flex min-h-0 flex-1 flex-col items-center justify-center gap-6 px-6 text-center text-white" style={{ background: 'radial-gradient(120% 80% at 50% 0%, #17130d 0%, #0a0a0b 60%, #060607 100%)' }}>
          <p className="font-heading text-lg font-semibold text-white/90">Parle-moi.<br /><span className="text-white/50 text-sm font-normal">Je réponds et j’agis en direct.</span></p>
          <div className="flex h-24 items-end justify-center gap-1.5">
            {voiceState === 'speaking' ? Array.from({ length: 9 }).map((_, i) => <span key={i} className="w-1.5 rounded-full bg-[#F59E42]" style={{ height: 56, transformOrigin: 'center', animation: `apBar .9s ease-in-out ${i * 0.08}s infinite` }} />)
              ...
              : <div className="h-20 w-20 rounded-full border-2 border-[#F59E42]/40" style={{ animation: 'apGlow 2s ease-in-out infinite' }} />}
          </div>
          <div className="min-h-[48px] w-full space-y-1.5">
            {heard && <p className="text-sm text-white/45">« {heard} »</p>}
            {msgs.length > 0 && msgs[msgs.length - 1].role === 'assistant' && <p className="text-[15px] leading-snug text-white">{msgs[msgs.length - 1].text}</p>}
          </div>
          {(() => { const last = msgs[msgs.length - 1]; return last?.pending ? (
            <div className="w-full rounded-2xl border border-[#F59E42]/30 bg-white/[0.05] p-3 text-left">
              ...
              <p className="mt-1.5 text-[11px] text-white/40">Dis « oui » pour confirmer.</p>
            </div>
          ) : null })()}
          {/* contrôles bas */}
          <div className="mt-1 flex items-center gap-6">
            ...
            <button onClick={() => { if (vsRef.current === 'speaking') { try { window.speechSynthesis?.cancel() } catch {}; setVS('listening') } else startRec() }} className="relative grid h-16 w-16 place-items-center rounded-full">
              {(voiceState === 'listening' || voiceState === 'speaking') && <><span className="absolute inset-0 rounded-full border border-[#F5A623]" style={{ animation: 'apRing 1.8s ease-out infinite' }} /><span className="absolute inset-0 rounded-full border border-[#F5A623]" style={{ animation: 'apRing 1.8s ease-out .9s infinite' }} /></>}
              <span className="relative grid h-16 w-16 place-items-center rounded-full bg-[#F5A623] text-black" style={{ boxShadow: '0 0 34px rgba(245,166,35,.45)' }}>{voiceState === 'thinking' ? <Loader2 className="h-6 w-6 animate-spin" /> : voiceState === 'speaking' ? <Volume2 className="h-6 w-6" /> : <Mic className="h-6 w-6" />}</span>
            </button>
            <button onClick={onClose} className="grid h-11 w-11 place-items-center rounded-full bg-white/10 text-white/80 hover:bg-white/15" title="Fermer"><X className="h-5 w-5" /></button>
          </div>
        </div>
      ) : (
        /* ─── Vue chat écrit ─── */
        <>
          ...
          <form onSubmit={e => { e.preventDefault(); send() }} className="flex items-end gap-2 border-t border-black/5 bg-white px-3 py-2.5">
            <button type="button" onClick={toggleDictation} aria-label={dictating ? 'Arrêter la dictée' : 'Dicter'} title={dictating ? 'Arrêter la dictée' : 'Dicter le message'}
              ...
              {dictating ? <Square className="h-4 w-4 fill-current" /> : <Mic className="h-[18px] w-[18px]" />}
            </button>
